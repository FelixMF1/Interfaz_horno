...
def graficar():
    ax.clear()
    x = np.arange(0, int(presion_values.get()))
    y = (2*x**2-50)
    ax.plot(x, y)
    max_y = np.max(y)
...
#TEXTO DE PRESION
presion_label = Label(columna_izquierda, text="PRESION:", font=("Roboto",15),bg="beige").place(relx=0.1,rely=0.75)
#gas_label.pack(pady=8, padx=10)

#LISTA DE LOS VALORES DE LAS PRESIONES
presion_values = StringVar()
presion_values.set("10")
lista_desplegable = ttk.Combobox(columna_izquierda, width=17, values=["10", "12", "14", "16", "18", "20"], textvariable=presion_values).place(relx=0.3,rely=0.77)
